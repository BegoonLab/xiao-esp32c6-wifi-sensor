...
void init_bme(void) {
    ...
    ESP_ERROR_CHECK(bme680_init_desc(&sensor,
                                     CONFIG_BME680_I2C_ADDR,
                                     CONFIG_BME680_I2C_PORT,
                                     CONFIG_BME680_I2C_SDA,
                                     CONFIG_BME680_I2C_SCL)
    );
    ESP_ERROR_CHECK(bme680_init_sensor(&sensor));
    ESP_ERROR_CHECK(bme680_use_heater_profile(&sensor, BME680_HEATER_NOT_USED));
    ESP_ERROR_CHECK(bme680_set_oversampling_rates(&sensor,
                                                  BME680_OSR_16X,
                                                  BME680_OSR_16X,
                                                  BME680_OSR_16X)
    );
    ...
    memset(&sensor, 0, sizeof(sensor));
    ESP_ERROR_CHECK(bmp280_init_desc(&sensor,
                                     CONFIG_BME280_I2C_ADDR,
                                     CONFIG_BME280_I2C_PORT,
                                     CONFIG_BME280_I2C_SDA,
                                     CONFIG_BME280_I2C_SCL)
    );
